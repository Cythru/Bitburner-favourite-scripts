...
export function sparkline(data, width) {
  ...
  // ── Downsample to target width using max-pooling ──
  // If we have 120 data points and width=40, step=3 means each
  // output character represents a bucket of 3 data points.
  //
  // Max-pooling (vs step-sampling): instead of picking one sample
  // per bucket, we take the MAXIMUM value in each bucket.
  // This ensures peaks always appear in the chart regardless of
  // where a sample would happen to fall. For a net worth chart,
  // this means highs are always visible.
